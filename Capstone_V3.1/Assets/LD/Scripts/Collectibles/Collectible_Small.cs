...
/// <summary>
/// Small collectible that when collected adds to the player's small collectible count. Note: Collectible means that it is considered in the Save/Spawn system, and will be propogated if saved on level load
...
public class Collectible_Small : MonoBehaviour {
    ...
    /// <summary>
    /// Pickup the small collectible, play the sound effect, update GUI and deactivate it
    /// </summary>
    void Pickup()
    {
        SoundManager.instance.PlaySound2DOneShot(sfx_Collect, 1f, true);  //Play 2D Sound Effect
        playerManager.localPlayerData.smallCollectibles++;     //Add 1 to our Collectible static variable
    ...
    /// <summary>
    /// Collect the small collectible, saving it into the player's inventory
    /// </summary>
    void Collect()
    {
